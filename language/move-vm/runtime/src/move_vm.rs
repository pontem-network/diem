...
use move_vm_types::natives::balance::{NativeBalance, ZeroBalance};
...
impl MoveVM {
    ...
    pub fn new_session<'r, R: RemoteCache>(&self, remote: &'r R) -> Session<'r, '_, R> {
        self.runtime.new_session(remote, Box::new(ZeroBalance))
    }

    pub fn new_session_with_balance<'r, R: RemoteCache>(&self, remote: &'r R, balance: Box<dyn NativeBalance>) -> Session<'r, '_, R> {
